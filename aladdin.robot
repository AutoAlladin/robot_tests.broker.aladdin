...
Оновити сторінку з тендером
    ...
    Comment    ${next_page_load_count}    Evaluate    ${page_load_count}+${1}
    Comment    Set Suite Variable    ${page_load_count}    ${next_page_load_count}
    Comment    ${is_load_before_crash}=    Evaluate    ${page_load_count}>4
    Comment    Run Keyword If    ${is_load_before_crash}    Close All Browsers
    Comment    Run Keyword If    ${is_load_before_crash}    Aladdin.Підготувати клієнт для користувача    ${username}
    Comment    Run Keyword If    ${is_load_before_crash}    Search tender    ${username}    ${tender_uaid}
    Comment    Run Keyword If    ${is_load_before_crash}    Set Suite Variable    ${page_load_count}    ${1}
    Comment    Load Tender    ${apiUrl}/publish/SearchTenderByGuid?guid=ac8dd2f8-1039-4e27-8d98-3ef50a728ebf&tenderid=${tender_uaid}
    Comment    Switch Browser    1
    Reload Page
